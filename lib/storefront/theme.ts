...
export function resolveThemeFromStorefront(
	storefront?: PublicStorefront | StoreThemeConfig | null
): StorefrontThemeTokens {
	const parsed = storefront?.theme_config as StoreThemeConfig | undefined; // Renamed from StorefrontThemeConfig
	const palette: StorefrontThemePalette = parsed?.palette ?? {};

	const primaryColor = coerceColor(parsed?.primaryColor, DEFAULT_THEME.accent);
	...
	return {
		...
		surfaceMuted: coerceColor(palette.surfaceMuted, DEFAULT_THEME.surfaceMuted),
