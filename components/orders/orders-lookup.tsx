...
export function OrdersLookup() {
	const t = useTranslations("Storefront.Orders.lookup");
	...
	return (
		<section className="mx-auto w-full max-w-3xl space-y-6 rounded-3xl border border-(--store-border) bg-(--store-surface) p-6 shadow-sm">
			<header className="space-y-2 text-center">
				...
				<p className="text-sm text-(--store-text-muted)">{t("description")}</p>
			</header>
			...
			{order ? <OrderDetails order={order} t={t} /> : null}
		</section>
	);
}

function OrderDetails({ order, t }: { order: PublicOrder; t: any }) {
	return (
						...
						>
							<div>
								<p className="text-sm font-medium text-(--store-text)">
									{item.product?.name ?? `${t("item")} ${item.id}`}
								</p>
